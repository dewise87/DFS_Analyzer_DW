...
def test_migration_runner_is_idempotent(tmp_path: Path) -> None:
    ...
    assert records[15][0] == 16
    assert records[15][1] == "0016_ownership_model.sql"
    assert len(records[15][2]) == 64
    assert records[17][0] == 18
    assert records[17][1] == "0018_claim_grading.sql"
    assert len(records[17][2]) == 64
