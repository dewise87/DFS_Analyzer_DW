-- Widen `ops_runs.step` for the Tuesday results lane (`na-ops results`).
--
-- SQLite cannot alter a CHECK constraint, so preserve the complete append-only history
-- while rebuilding the table with the five new step names.
