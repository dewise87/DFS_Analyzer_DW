-- Add the optional Stokastic component-projection step to the slate lane.
-- SQLite cannot alter a CHECK constraint, so preserve the append-only history while
-- rebuilding the table, index, and guards exactly as the preceding lane migrations do.
...
CREATE TABLE ops_runs_rebuilt (
    ...
    step TEXT NOT NULL CHECK(
        step IN (
            ...
            'slate_salaries', 'slate_stats', 'slate_odds', 'slate_weather',
            'slate_projections', 'slate_episodes', 'slate_features', 'slate_build',
            'slate_memo', 'slate_simulate',
