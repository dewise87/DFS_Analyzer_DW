-- Widen `ops_runs.step` for the slate lane (`na-ops slate`).
--
-- SQLite cannot alter a CHECK constraint, so the table is rebuilt: the triggers and the
-- index are dropped, a widened table is created, every existing row is copied verbatim,
-- and the guards are recreated. The four batch step names are unchanged, so the history
-- `na-ops status` already reads survives byte for byte.
--
-- `batch_run_id` keeps its name and now identifies one invocation of either lane
-- (`ops-…` for the batch lane, `slate-…` for the slate lane). Renaming it would break
-- every reader of the history this migration is trying to preserve.
...
CREATE TABLE ops_runs_rebuilt (
    ...
    step TEXT NOT NULL CHECK(
        step IN (
            'collect', 'purge', 'extract', 'nflverse_refresh',
